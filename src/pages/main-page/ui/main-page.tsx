...
export function MainPage({offersList}: MainPageProps): JSX.Element {
  const [activeCity, setActiveCity] = useState<CityNameType>(CITIES_LIST[0].name);
  const activeCityHandler = (name: CityNameType):void => setActiveCity(name);
  const activeCityOffersList = offersList.filter((offer)=> offer.city.name === activeCity);
  ...
  return (
    <div className="page page--gray page--main">
      <main className={`page__main page__main--index ${isEmptyOffersList ? 'page__main--index-empty' : ''}`}>
        ...
        <LocationTabs activeCity={activeCity} onActiveCityHandler={activeCityHandler}/>
        {!isEmptyOffersList
          ? <MainPageBody activeCityOffersList={activeCityOffersList} activeCity={activeCity}/>
          : <MainPageEmpty/>}
      </main>
    </div>
  );
